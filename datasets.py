...
class ConformationDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        data = self.custom_clone(self.data[idx])
...
class PackedConformationDataset(ConformationDataset):
    ...
    def _pack_data_by_mol(self):
        ...
        for k, v in self._packed_data.items():
            data_dict = v[0].__dict__.copy()  # 转换为字典
            data = Data()

            # 手动复制需要的数据
            for key in ['x', 'edge_index', 'edge_attr', 'smiles', 'idx', 'edge_type', 'pos', 'normal', 'face', 'atom_type', 'rdmol', 'nx', 'totalenergy', 'boltzmannweight']:
                if key in data_dict:
                    setattr(data, key, data_dict[key])


            all_pos = [conf.__dict__['pos'] for conf in v]  # 从字典中获取位置信息
            ...
            if hasattr(data, 'totalenergy'):
                delattr(data, 'totalenergy')
            if hasattr(data, 'boltzmannweight'):
                delattr(data, 'boltzmannweight')
